...
<jsp:include page="kj/head.jsp" />
 <script type="text/javascript">
      	var list_url = "${basePath}catagroy/catagroy_getall.action";
    	//搜索
      	function doSearch(){
      		//重置页号
      		$("#pageNo").val(1);
  ...
  <!--内容详细-->
   ...
   <div class="content_style">
    <div class="Products_area_style">
     ...
     <form action="${basePath }catagroy/catagroy_add.action" method="post">
      <input name="catagroy.name" type="text"  placeholder="分类名称" class="add_text"/>
       <input name="dengji" type="hidden"  value="1" />
      ...
      <table>
       <thead>
        <tr class="title">
         ...
         <td class="title_name">一级分类id</td>
         <td class="inventory">一级分类名称</td>
         <td class="status">二级分类数</td>
         <td class="operating">操作</td>
         </tr>
        </thead>
       <tbody>
       
       <c:forEach items="${cset}" var="g" >
       
        <tr>
          <td><input name="" type="checkbox" value="" /></td>
          <td><a href="${basePath }catagroy/catagroy_get2.action?catagroy.caid=${g.caid}">${g.caid}</a></td>
          <td><a href="${basePath }catagroy/catagroy_get2.action?catagroy.caid=${g.caid}">${g.name }</a></td>
          <td>${g.num}</td>
          ...
          <a href="#" class="fzxx_btn">复制信息</a><a href="商品专区-评价管理.html" class="pjgl_btn">评价管理</a><a href="#" class="szkc_btn">设置库存</a><a href="${basePath }catagroy/catagroy_delete.action?catagroy.caid=${g.caid}" class="sj_btn">删除</a></td>
        </tr>
       </c:forEach>
